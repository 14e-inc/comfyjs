...
def store_command_result(cmd, logger, dry_run=False, stdout_file=None):
    ...
    def virt_install_with_cloud_init(vm_name, cloud_init_file_path, dry_run=True):
        """Installs a VM using virt-install with cloud-init configuration."""
        cmd = [
            "virt-install",
            "--name", vm_name,
            "--disk", f"path=/var/lib/libvirt/images/{vm_name}.qcow2,format=qcow2",
            "--ram", "2048",
            "--vcpus", "2",
            ...
            "--import",
            "--cloud-init", f"user-data={cloud_init_file_path}"
        ]
        return run_command(cmd, logger, dry_run=dry_run)
